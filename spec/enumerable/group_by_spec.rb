...
RSpec.describe "Enumerable#group_by" do
  it "returns a Hash such as each key is a returned value from the block and \
      each value is an array of those elements for which the block returned that key" do
    enum = EnumerableSpecs::Numerous.new("a", "b", "abc")
    expect(enum.group_by { |s| s.size }).to eq({ 1 => ["a", "b"], 3 => ["abc"]})
  end
  ...
  it "returns an Enumerator if called without a block" do
    ...
    expect(enum.group_by.each { |s| s.size }).to eq({ 1 => ["a", "b"], 3 => ["abc"]})
  end
  ...
  context "when #each yields multiple" do
    it "gathers whole arrays as elements when #each yields multiple" do
      ...
      expect(enum.group_by { |i| i }).to eq({ [1, 2] => [[1, 2]],
                        [6, 7, 8, 9] => [[6, 7, 8, 9]],
                        [3, 4, 5] => [[3, 4, 5]] })
    end
    ...
    describe "when no block is given" do
      describe "returned Enumerator" do
        it "size returns nil" do
          ...
          expect(enum.group_by.size).to eq(nil)
        end
      end
    end
  end
