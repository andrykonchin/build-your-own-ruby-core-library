...
RSpec.describe "Range#reverse_each" do
  ...
  it 'passes each element of self to the block in reverse order' do
  ...
  it "iterates indefinitely if beginingless range with Numeric end" do
    range = Range.new(nil, 2)
    range.reverse_each { |e| break if e < 0; ScratchPad << e }
    expect(ScratchPad.recorded).to eq([2, 1, 0])
  end

  it "raises TypeError if beginingless range with non-Numeric end" do
  ...
  describe 'infinit range' do
    describe 'when no block is given' do
      describe 'returned Enumerator' do
        ...
        it 'size returns Float::INFINITY if beginingless range with Numeric end' do
          enum = Range.new(nil, 1).reverse_each
          expect(enum.size).to eq(Float::INFINITY)
        end

        it 'size raises TypeError if beginingless range with non-Numeric end' do
